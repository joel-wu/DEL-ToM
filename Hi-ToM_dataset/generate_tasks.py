    ...
    for world in world_paths:
        ...
        for length_of_story in num_chapters:
            ...
            for i in range(1, n+1):
                ...
                # --------------
                story, belief_trace = task.generate_story_qs_at_end(
                    w,
                    length_of_story,
                    tasks_per_length[length_of_story - 1],
                    num_agents=5,
                    num_locations=3,
                    statement_noise=noise,
                    order=0,
                    exist_tell_in_story=exist_tell_in_story
                )
                ...
                for mode in modes:
                    ...
                    for order_of_story in orders:
                        ...
                        # --------- save belief_trace ----------
                        trace_path = path.replace(f'order_{order_of_story}.txt', 'trace.jsonl')
                        with open(trace_path, 'w', encoding='utf-8') as ft:
                            for snap in belief_trace:
                                ft.write(json.dumps(snap) + '\n')
                        # -----------------------------------------------------
...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        #default='data_ToMh_Train', help='Output directory path'
